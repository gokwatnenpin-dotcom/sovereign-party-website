...
export default function Leadership() {
  const ref = useReveal();
  return (
    <section id="leadership" aria-labelledby="leadership-heading" className="bg-off-white py-16 lg:py-24">
      <div ref={ref} className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="reveal max-w-2xl">
          <SectionLabel>Leadership</SectionLabel>
          <h2 id="leadership-heading" className="mt-4 font-display text-[32px] font-bold text-navy sm:text-[36px]">
        ...
        <article className="reveal mt-10 grid overflow-hidden rounded-[8px] bg-navy shadow-[0_4px_24px_rgba(0,0,0,0.12)] lg:grid-cols-2">
          <div className="relative min-h-[320px] lg:min-h-[480px]">
            <img
              ...
              loading="lazy"
            />
            <span className="absolute left-5 top-5 rounded-[4px] bg-gold px-3 py-1.5 text-[11px] font-bold uppercase tracking-[1.5px] text-navy">
          ...
          <div className="flex flex-col justify-center p-7 sm:p-10 lg:p-12">
            ...
            <p className="mt-1 text-sm font-semibold uppercase tracking-[1.5px] text-white/60">
              Presidential Candidate &amp; Party Chairman
            </p>
            <blockquote className="mt-6 border-l-4 border-gold pl-5 font-display text-[22px] font-bold italic leading-snug text-gold-light sm:text-2xl">
              “A nation that invests in its people cannot fail.”
            </blockquote>
            <p className="mt-6 text-[15px] leading-[1.75] text-white/75">
            ...
            <div className="mt-7 flex flex-wrap gap-3">
              <span className="rounded-[4px] bg-white/10 px-3 py-1.5 text-[12px] font-semibold uppercase tracking-[1px] text-white/80">
          ...
          {LEADERS.map((p, i) => (
            ...
            >
              <div className="relative h-64 overflow-hidden">
                ...
                />
                <span className="absolute bottom-3 left-3 rounded-[4px] bg-navy/90 px-3 py-1 text-[11px] font-bold uppercase tracking-[1.5px] text-gold">
                  {p.role}
                </span>
              </div>
              <div className="p-6">
                <h3 className="font-display text-[22px] font-bold text-navy">{p.name}</h3>
                <p className="mt-0.5 text-[12px] font-semibold uppercase tracking-[1.5px] text-crimson">
                  {p.role}
                </p>
                <p className="mt-3 text-[14px] leading-[1.7] text-muted">{p.desc}</p>
                <a
                  href={`https://x.com/${p.handle.replace("@", "")}`}
                  target="_blank"
                  rel="noreferrer"
                  className="mt-4 inline-flex items-center gap-2 text-[13px] font-semibold text-navy transition-colors hover:text-crimson"
