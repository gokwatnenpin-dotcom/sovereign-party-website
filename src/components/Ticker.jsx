...
export default function Ticker() {
  ...
  return (
    <div
      ...
      className="overflow-hidden border-y border-gold/20 bg-navy-mid py-3.5"
    >
        ...
        {[0, 1].map((half) => (
          <div key={half} className="flex shrink-0 items-center" aria-hidden={half === 1}>
            {row.slice(half * TICKER_ITEMS.length, half * TICKER_ITEMS.length + TICKER_ITEMS.length).map((item, i) => (
              <span
                key={`${half}-${i}`}
                className="flex items-center whitespace-nowrap text-[13px] font-semibold uppercase tracking-[1.2px] text-gold"
              >
